...
//        goto skip;
    ...
    alg = EVP_SIGNATURE_fetch(NULL, "RSA-SHA256", "?provider=ibmca");
    if (alg == NULL) {
        fprintf(stderr, "EVP_SIGNATURE_fetch for RSA-SHA256 failed\n");
        goto out;
    }
    ...
    /* Digest-VerifyMessage with default provider */
    ctx = EVP_PKEY_CTX_new_from_pkey(NULL, rsa_pkey, "provider=default");
    if (ctx == NULL) {
        fprintf(stderr, "EVP_PKEY_CTX_new_from_pkey failed\n");
        goto out;
    ...
    alg = EVP_SIGNATURE_fetch(NULL, "RSA-SHA256", "provider=default");
    if (alg == NULL) {
        fprintf(stderr, "EVP_SIGNATURE_fetch for RSA-SHA256 failed\n");
        goto out;
    }
    ...
    /* Digest-VerifyMessage with IBMCA provider */
    ctx = EVP_PKEY_CTX_new_from_pkey(NULL, rsa_pkey, "?provider=ibmca");
    if (ctx == NULL) {
        fprintf(stderr, "EVP_PKEY_CTX_new_from_pkey failed\n");
        goto out;
    ...
    /* Digest-SignMessage with default provider */
    ctx = EVP_PKEY_CTX_new_from_pkey(NULL, rsa_pkey, "provider=default");
    if (ctx == NULL) {
        fprintf(stderr, "EVP_PKEY_CTX_new_from_pkey failed\n");
        goto out;
